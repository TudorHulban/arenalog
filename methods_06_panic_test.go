...
func TestLogger_JSON_Panic(t *testing.T) {
	...
	cmd := exec.CommandContext(
		context.Background(),
		os.Args[0],
		"-test.run=^TestLogger_JSON_Panic$",
	)
...
func TestLogger_Raw_Panic(t *testing.T) {
	...
	require.NoError(t, logSet.First().Contains(1, "PANIC"))
	require.NoError(t, logSet.First().Contains(1, "core system panic"))
}
