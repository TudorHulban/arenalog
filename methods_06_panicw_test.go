...
func TestLogger_JSON_Panicw(t *testing.T) {
	...
	if os.Getenv("BE_PANICKING") == "1" {
		...
		l.Panicw("core system panic", "floor")
	}
	...
	cmd := exec.CommandContext(
		context.Background(),
		os.Args[0],
		"-test.run=^TestLogger_JSON_Panicw$",
	)
	...
	require.NoError(t, cleanLogset.HasKeyWithValueLike("msg", "odd_args", 1))
}
...
func TestLogger_Raw_Panicw(t *testing.T) {
	...
	cmd := exec.CommandContext(
		context.Background(),
		os.Args[0],
		"-test.run=^TestLogger_Raw_Panicw$",
	)
